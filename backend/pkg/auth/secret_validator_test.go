...
func TestTelegramInitDataValidation(t *testing.T) {
	...
	validator := NewAppValidator(botToken, ProviderTelegram)
	...
	t.Run("ValidateWebAppInitData with signature (Ed25519)", func(t *testing.T) {
		...
		result, err := validator.ValidateWebAppInitData(ctx, testInitData)
		if err != nil {
...
func TestTelegramInitDataValidationWithFreshData(t *testing.T) {
	...
	result, err := validator.ValidateWebAppInitData(ctx, initDataWithoutSignature)
	if err != nil {
	...
	result2, err2 := validator.ValidateWebAppInitData(ctx, initDataWithSignature)
	if err2 != nil {
...
func buildInitData(userJSON string, authDate int64, chatType, chatInstance, signature string) string {
	result := "user=" + urlEncode(userJSON) +
		"&chat_instance=" + chatInstance +
		"&chat_type=" + chatType +
		"&auth_date=1773181118"
...
// urlEncode простая URL кодировка
...
func TestValidateThirdPartyLogic(t *testing.T) {
	...
	validator := NewAppValidator("7342037359:AAHI25ES9xCOMPokpYoz-p8XVrZUdygo2J4", ProviderTelegram)
	ctx := context.Background()

	result, err := validator.ValidateWebAppInitData(ctx, exampleInitData)
	if err != nil {
		t.Logf("Validation error: %v", err)
