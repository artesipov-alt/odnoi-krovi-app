...
### Сценарий 5: профиль питомца неполный (нет вакцинаций/обработок)

Точка отсчёта: `pets.created_at` (момент создания питомца).
Условие: у питомца не заполнена хотя бы одна из четырёх дат в обработках
(бешенство, инфекции, эктопаразиты, дегельминтизация) или запись обработок
вообще не создана — это устранимые стоп-факторы донора.

| Название | Тип (код) | Кому | Первое | Повтор | Остановка |
|---|---|---|---|---|---|
| «Питомец может стать героем» | `pet_no_treatments` | владельцу питомца | 24 ч после создания питомца | каждые 48 ч, **без лимита** | заполнил данные по вакцинации и обработкам |

Если у владельца несколько неполных питомцев — один пуш за цикл
(по самому старому неполному питомцу), чтобы не заваливать дубликатами.
...
### Сценарий 5: профиль питомца неполный

**«Питомец может стать героем»** (`pet_no_treatments`) — через 24 часа после создания
питомца, если не заполнены вакцинации/обработки; повторяется каждые 48 часов без лимита:

> Ваш питомец может стать героем!
>
> Укажите данные по вакцинации и обработкам в профиле питомца, чтобы он мог помогать
> другим в качестве донора и получать за это награды.
