...
// The UtmHistoryFunc type is an adapter to allow the use of ordinary function as a Querier.
type UtmHistoryFunc func(context.Context, *ent.UtmHistoryQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UtmHistoryFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UtmHistoryQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UtmHistoryQuery", q)
}

// The TraverseUtmHistory type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUtmHistory func(context.Context, *ent.UtmHistoryQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUtmHistory) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUtmHistory) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UtmHistoryQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UtmHistoryQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UtmHistoryQuery:
		return &query[*ent.UtmHistoryQuery, predicate.UtmHistory, utmhistory.OrderOption]{typ: ent.TypeUtmHistory, tq: q}, nil
	default:
